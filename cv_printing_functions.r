...
create_CV_object <-  function(data_location,
  ...
  if(is_google_sheets_location){
    ...
    cv$prog        <- read_gsheet(sheet_id = "programming")
    cv$infosec        <- read_gsheet(sheet_id = "info_sec")
    cv$netsec        <- read_gsheet(sheet_id = "network_security")
    cv$os        <- read_gsheet(sheet_id = "operating_systems")
...
#' @description Skills section with icons, similar to contact info (instead of bars)
print_skills <- function(cv){
  glue::glue_data(
    cv$skills,
    "- <i class='fa fa-{icon}'></i> {skill}"
  ) %>% print()
  
  invisible(cv)
}#' @description Skills section with icons, similar to contact info (instead of bars)
print_prog <- function(cv){
  glue::glue_data(
    cv$prog,
    "- {skill}"
  ) %>% print()
